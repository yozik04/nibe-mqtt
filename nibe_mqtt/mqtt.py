...
class MqttConnection:
    def __init__(self, handler: MqttHandler, conf: dict):
        ...
        self._client.will_set(self._availability_topic, "offline", retain=conf['retain_availability'])
    ...
    def publish_coil_state(self, coil: Coil):
        self._client.publish(self._get_coil_state_topic(coil), coil.value, retain=self._conf['retain_state'])
    ...
    def publish_discovery(self, coil: Coil, device_info: dict):
        ...
        self._client.publish(f"{self._conf['discovery_prefix']}/{component}/{device_id}/{coil.name}/config", json.dumps(config), retain=self._conf['retain_state'])