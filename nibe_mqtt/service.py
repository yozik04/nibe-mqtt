...
class Service(MqttHandler):
    ...
    async def write_coil(self, coil_data: CoilData) -> None:
        ...
        try:
            coil_value = coil_data.value
            await self.connection.write_coil(coil_data)
            if coil_value == coil_data.value:  # if coil value did not change while we were writing, just publish to MQTT
                self.on_coil_update(coil_data)
                refresh_required = False
            else:  # if value has changed we do not know what is the current state
                logger.info(
                    f"{coil_data.coil.name} value has changed while we were writing: {coil_value} -> {coil_data.value}"
                )
